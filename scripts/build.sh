...
# What to build
BUILD_TARGETS=`find poky/meta-openembedded/meta-python poky/meta/recipes-devtools/python -name '*.bb' | xargs -n1 basename | grep -v systemd | grep -v networkmanager | grep -v blivet | grep -v pam | cut -d '_' -f 1 | tr '\n' ' '`
